...
public:
    /* Se obtienen las variables correspondientes a las fisicas a implementar a cada objeto */
    Fisicas(float posX_,float posY_,float velX_,float velY_,float masa_,float radio_,float K_,float e_);
    Fisicas(float posX_, float posY_, float velX_, float velY_, float masa_, float radio_, float K_, float F_, float G);
    ~Fisicas();
    void Formulas();
